...
time_richcompare(PyObject *self, PyObject *other, int op)
{
    ...
    /* The hard case: both aware with different UTC offsets */
        ...
        if (diff > -2 && diff < 2) {
            /* Microseconds in time values and offsets can contribute up to
            1.999998 seconds to the diff in either way. If difference is within
            that margin, consider microseconds as well. */
            diff = diff * 1000000 + TIME_GET_MICROSECOND(self) -
                GET_TD_MICROSECONDS(offset1) - TIME_GET_MICROSECOND(other) +
